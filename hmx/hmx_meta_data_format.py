...
def GetDetectInfo(metadata):
    ...
    for i in range(fd_num_of_detection):
        ...
        register_id_offset = fr_offset + struct_FR_INFO.register_id.offset
        """
        print('x_offset = %d' % x_offset)
        print('y_offset = %d' % y_offset)
        print('w_offset = %d' % w_offset)
        ...
        register_id = int.from_bytes(metadata[register_id_offset : register_id_offset+4], byteorder='little', signed=False)
        #print('face (yaw, pitch, roll) = (%d, %d, %d)' % (face_yaw, face_pitch, face_raw) )
        print('register_id = %d' % register_id)
        #print('x = %d, y = %d, width = %d, height = %d, score = %d, confidence = %d' % (x, y, width, height, score, confidence))
        if width != 0:
            fd_info = {'x':x, 'y':y, 'width':width, 'height':height, 'score':score, 'yaw':face_yaw, 'pitch':face_pitch, 'zone':zone, 'register_id':register_id}
